...
public interface BasicSwitch<K, T, U, R>
    ...
    default <K2> BasicSwitch<K2, T, U, R> mapKey(CheckedPredicate.Adapter<? super K, ? extends K2> conditionAdapter) {
        return asBasicSwitch(Switch.super.mapKey(conditionAdapter));
    }

    @Override
    default <U2> BasicSwitch<K, T, U2, R> mapTarget(BiFunction<? super T, ? super U2, ? extends U> valueMapping) {
        return asBasicSwitch(Switch.super.mapTarget(valueMapping));
    }
