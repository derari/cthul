...
public class SwitchOperations {
    ...
    public static <K, K2, T, U, R> Switch<K2, T, U, R, ?, ?, ?> mapKey(Switch<K, T, U, R, ?, ?, ?> instance, CheckedPredicate.Adapter<? super K, ? extends K2> conditionAdapter) {
        return instance.wrapWith(delegate -> new ConditionAdaptingSwitch<>(delegate, conditionAdapter));
    }

    public static <K, K2, T, U, R> Switch<K2, T, U, R, ?, ?, ?> mapKey(Switch<K, T, U, R, ?, ?, ?> instance, Function<? super K, ? extends K2> keyMapping) {
        return mapKey(instance, CheckedPredicate.Adapter.map(keyMapping));
    }

    public static <K, T, T2, U, R> Switch<K, T2, U, R, ?, ?, ?> mapSource(Switch<K, T, U, R, ?, ?, ?> instance, Function<? super T, ? extends T2> valueMapping) {
