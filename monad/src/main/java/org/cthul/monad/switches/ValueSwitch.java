...
public class ValueSwitch<K, T>
    ...
    @Override
    public <X extends Exception> IdentityCase<T, BasicSwitch.Identity<K, T, T>> ifFalse(CheckedPredicate<? super K, X> condition) throws X {
        if (!condition.test(key)) {
            return new MatchCase<>(value, this::matched);
        }
        return this;
    ...
    @Override
    public <X extends Exception> BasicSwitch.Identity<K, T, T> map(CheckedFunction<T, ? extends T, X> function) throws X {
        return basic;
    }
    ...
    protected static class MatchCase<T, S> implements Switch.IdentityCase<T, S> {
        ...
        @Override
        public <X extends Exception> S map(CheckedFunction<T, ? extends T, X> function) throws X {
