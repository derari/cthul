...
public class CacheControlTest {
    ...
    public void testParseDuration() {
        Duration duration = new CacheControl.Builder().parse("max-age=15").getMaxAge();
        assertThat(duration.getSeconds(), is(15L));
    }
    
    @Test
    public void testParseDurationDay() {
        Duration duration = new CacheControl.Builder().parse("max-age=2D").getMaxAge();
        assertThat(duration.getSeconds(), is(2*86400L));
    }
    
    @Test
    public void testParseDurationMinutes() {
        Duration duration = new CacheControl.Builder().parse("max-age=2M").getMaxAge();
        assertThat(duration.getSeconds(), is(120L));
    }
    
    @Test
    public void testParseDurationDayMinutes() {
        Duration duration = new CacheControl.Builder().parse("max-age=2D2M").getMaxAge();
        assertThat(duration.getSeconds(), is(2*86400L + 120L));
    }
    
    @Test
    public void testParseDurationDayTMinutes() {
        Duration duration = new CacheControl.Builder().parse("max-age=2DT2M").getMaxAge();
        assertThat(duration.getSeconds(), is(2*86400L + 120L));
    }
    
    @Test
    public void testParseDurationDaySeconds() {
        Duration duration = new CacheControl.Builder().parse("max-age=2D2").getMaxAge();
        assertThat(duration.getSeconds(), is(2*86400L + 2L));
    }
    
    @Test
    public void testParseDurationDoubleNegation() {
        Duration duration = new CacheControl.Builder().parse("max-age=-P-1H").getMaxAge();
