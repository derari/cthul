...
public class SimpleMatchLexerBuilder<T extends InputMatch<?>> extends SimpleLexerBuilder<T, MatchResult, TokenInput<T>> {
    ...
    protected static class RuleSetBuilder<T extends InputMatch<?>> extends SimpleLexerBuilder.RuleSetBuilder<T, MatchResult, TokenInput<T>, State<T>> {
        ...
        @Override
        protected StringRule<T> newStringRule(RuleKey key, int id, InputEval<? extends T, ? super MatchResult> eval, String string) {
            return new StringRule<>(key, id, eval, new String[]{string});
        }

        @Override
        protected StringRule<T> newStringRule(RuleKey key, int id, InputEval<? extends T, ? super MatchResult> eval, String... strings) {
            return new StringRule<>(key, id, eval, strings);
        }

        @Override
        protected RegexRule<T> newRegexRule(RuleKey key, int id, InputEval<? extends T, ? super MatchResult> eval, Pattern pattern) {
