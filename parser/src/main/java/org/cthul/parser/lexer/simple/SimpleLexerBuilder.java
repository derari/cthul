...
public abstract class SimpleLexerBuilder<Token, Match, I extends Input<?>> extends LexerBuilderBase<Token, Match, I> {
    ...
    @SuppressWarnings("OverridableMethodCallInConstructor")
    public SimpleLexerBuilder(SimpleLexerBuilder<Token, Match, I> source) {
        super(source);
        ruleSetBuilder = newRuleSetBuilder();
    ...
    protected static abstract class RuleSetBuilder<Token, Match, I extends Input<?>, S extends State<Token, I>> {
        ...
        protected void registerMatcher(RuleKey key) {
            if (registeredMatchers.add(key)) {
                lexerBuilder.addMatcher(createMatcher(key));
            }
        }
        ...
        public void addStringToken(RuleKey key, MatchEval<? extends Token, ? super Match> eval, String string) {
            addRule(key, newStringRule(key, nextId(), eval, string));
        }
        
        public void addStringToken(RuleKey key, MatchEval<? extends Token, ? super Match> eval, String... strings) {
            addRule(key, newStringRule(key, nextId(), eval, strings));
        }

        public void addRegexToken(RuleKey key, MatchEval<? extends Token, ? super Match> eval, Pattern pattern) {
            addRule(key, newRegexRule(key, nextId(), eval, pattern));
        }
        
        protected abstract Rule<? extends Token, ? super S> newStringRule(RuleKey key, int id, MatchEval<? extends Token, ? super Match> eval, String string);
        
        protected abstract Rule<? extends Token, ? super S> newStringRule(RuleKey key, int id, MatchEval<? extends Token, ? super Match> eval, String... strings);
        
        protected abstract Rule<? extends Token, ? super S> newRegexRule(RuleKey key, int id, MatchEval<? extends Token, ? super Match> eval, Pattern pattern);
    }
