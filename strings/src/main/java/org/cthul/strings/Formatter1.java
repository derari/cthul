 ...
 */
public class Formatter1 implements Flushable, Closeable {
    ...
    public static void Format(Message message, Appendable appendable) {
        new Formatter1(message, appendable).run();
    }
   ...
   private final Message message;
     ...
     *         formatting.
     */
    public Formatter1(Appendable appendable, FormatConfiguration conf) {
     ...
     *         is applied.
     */
    public Formatter1(Appendable appendable, Locale locale) {
        this(null, FormatConfiguration.getDefault().forLocale(locale), appendable);
    }
    
    protected Formatter1(Message message, Appendable appendable) {
        this(message, message.getConfiguration(), appendable);
    }
    
    private Formatter1(Message message, FormatConfiguration conf, Appendable appendable) {
    ...
    protected java.util.Formatter utilFormatter() {
        if (util == null) {
    ...
    protected Formatter1 append(char c) throws IOException {
        ensureOpen();
        if (uppercase) {
    ...
    protected Formatter1 append(CharSequence csq) throws IOException {
        ensureOpen();
        if (uppercase) {
    ...
    protected Formatter1 append(CharSequence csq, int start, int end) throws IOException {
        ensureOpen();
     ...
     */
    public Formatter1 format(final String format, final Object... args) {
     ...
     */
    public Formatter1 format(final Locale locale, final String format, final Object... args) {
    ...
    private void stringFormat(Matcher matcher, String argId, int argIndex) throws IOException {
        ...
        utilFormatter().format(format, tmp);
    ...
    private int customFormat(final Matcher matcher, final int index, 
        ...
        final boolean forceUppercase = Character.isUpperCase(f.charAt(0));
